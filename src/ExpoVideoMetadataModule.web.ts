	...
	async getVideoInfo(
		...
		try {
			...
			return {
				...
				bitRate:
					metadata.bitrate ||
					(fileSize && metadata.duration
						? Math.floor((fileSize * 8) / metadata.duration)
						: 0),
				fileSize,
				...
				isHDR: metadata.colorInfo
					? HdrDetector.isHdr(metadata.colorInfo)
					: null,
				codec: metadata.codec || "",
				fps: metadata.fps || 0,
				orientation: this.getOrientationFromMatrix(
					metadata.rotation,
					metadata.width,
					metadata.height,
				),
			...
			//console.log("Affected File", source?.name);
