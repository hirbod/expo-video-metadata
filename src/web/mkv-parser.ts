...
import type { WebMElement, VideoTrackMetadata, ParsedVideoMetadata } from '../ExpoVideoMetadata.types';
...
export class MKVParser extends WebMParser {
  ...
  protected parseVideoTrack(track: WebMElement): VideoTrackMetadata {
    ...
    try {
      ...
      if (video) {
        ...
        if (cropValues) {
          baseMetadata.displayAspectWidth = baseMetadata.width - (cropValues.left + cropValues.right);
          baseMetadata.displayAspectHeight = baseMetadata.height - (cropValues.top + cropValues.bottom);
        }

        // Handle aspect ratio type
        const aspectRatioType = this.findElement(video.data, MKVParser.MKV_ELEMENTS.AspectRatioType);
        if (aspectRatioType && aspectRatioType.data.length > 0) {
          this.applyAspectRatioType(aspectRatioType.data[0], baseMetadata);
        }

        // Handle stereo mode
        const stereoMode = this.findElement(video.data, MKVParser.MKV_ELEMENTS.StereoMode);
        if (stereoMode) {
  ...
  private parseCropValues(videoData: Uint8Array): { top: number; bottom: number; left: number; right: number } | null {
    try {
      const cropTop = this.findElement(videoData, MKVParser.MKV_ELEMENTS.PixelCropTop);
      const cropBottom = this.findElement(videoData, MKVParser.MKV_ELEMENTS.PixelCropBottom);
      const cropLeft = this.findElement(videoData, MKVParser.MKV_ELEMENTS.PixelCropLeft);
      const cropRight = this.findElement(videoData, MKVParser.MKV_ELEMENTS.PixelCropRight);
  ...
  private applyAspectRatioType(aspectRatioType: number, metadata: VideoTrackMetadata): void {
    // 0: free resizing
    // 1: keep aspect ratio
    // 2: fixed dimensions
  ...
  private applyStereoMode(stereoMode: number, metadata: VideoTrackMetadata): void {
