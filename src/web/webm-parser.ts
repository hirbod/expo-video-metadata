...
export class WebMParser {
 ...
 public async parse(): Promise<ParsedVideoMetadata> {
   ...
   const audioInfo = audioTrack ?
     ...
     { hasAudio: false, audioChannels: 0, audioSampleRate: 0, audioCodec: '', audioBitrate: undefined };
...
protected readElement(): WebMElement | null {
 ...
 protected parseVideoTrack(track: WebMElement): VideoTrackMetadata {
   ...
   try {
     ...
     if (codecId) {
       ...
       const codecPrivate = this.findElement(track.data, WebMParser.ELEMENTS.CodecPrivate);
     ...
     const bitrate = this.findElement(track.data, WebMParser.ELEMENTS.VideoBitrate);
     if (bitrate) {
 ...
 protected async findAudioTrack(data: Uint8Array): Promise<WebMElement | null> {
   try {
     ...
     while (localReader.remaining() >= 2) {
       ...
       if (id === WebMParser.ELEMENTS.TrackEntry) {
         const type = this.findElement(trackData, WebMParser.ELEMENTS.TrackType);
         if (type && type.data[0] === 2) { // 2 = audio track
           return {
             id,
             size: Number(size),
             data: trackData,
 ...
 protected parseAudioTrack(track: WebMElement): {
   ...
   try {
     ...
     if (audio) {
       const channelsElem = this.findElement(audio.data, WebMParser.ELEMENTS.Channels);
       if (channelsElem) {
         channels = this.readUintFromElement(channelsElem);
       }

       const sampleRateElem = this.findElement(audio.data, WebMParser.ELEMENTS.SamplingFrequency);
       if (sampleRateElem) {
         sampleRate = this.readUintFromElement(sampleRateElem);
       }

       // Get audio bitrate if available
       const bitrate = this.findElement(audio.data, WebMParser.ELEMENTS.AudioBitrate);
       if (bitrate) {
